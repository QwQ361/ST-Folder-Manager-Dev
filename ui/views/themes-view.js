...
export function renderThemesViewCore(deps) {
  ...
  const themeFavCount = themeNames.filter((n) => themeFavs.includes(n)).length;
  const themeFavNode = $(`
  ...
  const topFolders = sortResFolders("themes", getResTopLevelFolders("themes"));
  ...
  // 获取当前主题
  const currentThemeName = (getContext().powerUserSettings || {}).theme || null;
  ...
  if (!state.selectedThemeFolder) {
    ...
    for (const name of displayItems) {
      ...
      const isMSel =
        state.cfmMultiSelectMode && state.cfmMultiSelected.has(name);
      const isExpSel = state.cfmExportMode && state.cfmExportSelected.has(name);
      const isDelSel =
        state.cfmResDeleteMode && state.cfmResDeleteSelected.has(name);
      const isNoteSel =
        state.cfmThemeNoteMode && state.cfmThemeNoteSelected.has(name);
      const isRenameSel =
        state.cfmThemeRenameMode && state.cfmThemeRenameSelected.has(name);
      const isBgBindSel =
        state.cfmThemeBgBindMode && state.cfmThemeBgBindSelected.has(name);
        ...
        : state.cfmExportMode
          ...
          : state.cfmThemeNoteMode
            ...
            : state.cfmThemeRenameMode
              ...
              : state.cfmThemeBgBindMode
                ? `<div class="cfm-edit-checkbox ${isBgBindSel ? "cfm-edit-checked" : ""}"><i class="fa-${isBgBindSel ? "solid" : "regular"} fa-square${isBgBindSel ? "-check" : ""}"></i></div>`
      ...
      const bgBindHtml = bgBinding
        ...
        : "";
      const row = $(`
        <div class="cfm-row cfm-row-char ${isActive ? "cfm-rv-item-active" : ""} ${isDelSel ? "cfm-res-delete-row-selected" : ""} ${isExpSel ? "cfm-export-row-selected" : ""} ${isNoteSel ? "cfm-edit-row-selected" : ""} ${isRenameSel ? "cfm-edit-row-selected" : ""} ${isBgBindSel ? "cfm-edit-row-selected" : ""} ${isMSel ? "cfm-multisel-row-selected" : ""}" data-res-id="${escapeHtml(name)}" draggable="true">
            ${msCheckHtml}
      ...
      row.on("click", (e) => {
        ...
        if (state.cfmThemeBgBindMode) {
          toggleThemeBgBindItem(name, e.shiftKey);
          renderThemesView();
          return;
        }
    ...
    // 批量绑定背景工具栏
    prependThemeBgBindToolbar(rightList, renderThemesView);
    // 多选工具栏
