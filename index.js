...
jQuery(async () => {
  ...
  function ensureResourceSettings() {
    ...
    if (!extension_settings[extensionName].personaGroups)
    ...
    if (!extension_settings[extensionName].resourceFolderTree.personas)
      extension_settings[extensionName].resourceFolderTree.personas = {};
    // 正则标签页：全局正则的虚拟文件夹树和脚本分组
    if (!extension_settings[extensionName].regexFolderTree)
  ...
  function ensureSettings() {
    ...
    for (const p of extension_settings[extensionName].wiActivePresets) {
  ...
  function takeResSortSnapshot(type) {
    ...
    if (type === "personas" && personaSortSnapshot) return;
    const tree = getResFolderTree(type);
    const snap = {};
    ...
    else if (type === "personas") personaSortSnapshot = snap;
  ...
  function applyResSortToFolders(type, folderIds, mode) {
    ...
    else if (type === "backgrounds") bgSortDirty = true;
    else if (type === "personas") personaSortDirty = true;
  ...
  function revertResSort(type) {
    ...
    if (type === "presets") {
      ...
      personaSortSnapshot = null;
      personaSortDirty = false;
      ...
      if (currentResourceType === "chars") {
        ...
        for (const name of selected) {
          try {
            ...
            if (resp.ok) {
              ...
              // 如果删除的是默认背景，清除默认背景设置
              if (
                extension_settings[extensionName].defaultBackground === name
              ) {
                extension_settings[extensionName].defaultBackground = "";
              }
              // 从酒馆原生DOM中移除对应背景元素
        ...
        for (const avatarId of selected) {
          try {
            ...
            if (resp.ok) {
              // 清理 personas 和 persona_descriptions
              const pu = getContext().powerUserSettings;
              if (pu) {
              ...
              // 清理收藏
              const favs = extension_settings[extensionName].personaFavorites;
              ...
              $(".avatar-container")
                .filter(function () {
                  return $(this).attr("data-avatar-id") === avatarId;
                })
                .remove();
              success++;
            } else {
  ...
  function handleThemeBgLink(themeName) {
    ...
    const powerUserTheme = (getContext().powerUserSettings || {}).theme || null;
    ...
    if (existingBinding) {
      ...
      const overlay = $(
        '<div id="cfm-bglink-overlay" class="cfm-batch-overlay"></div>',
      );
      const dialog = $(`
      ...
      dialog
        .find("#cfm-bglink-close, #cfm-bglink-cancel")
        .on("click touchend", (e) => {
          e.preventDefault();
          e.stopPropagation();
          closeOverlay();
  ...
  function handleDefaultBgSetting() {
    ...
    const overlay = $(
      '<div id="cfm-defbg-overlay" class="cfm-batch-overlay"></div>',
    );
    const dialog = $(`
      <div class="cfm-batch-popup" style="max-width:420px;">
    ...
    const closeOverlay = () => overlay.remove();
    dialog
      .find("#cfm-defbg-close, #cfm-defbg-cancel")
      .on("click touchend", (e) => {
        e.preventDefault();
        e.stopPropagation();
        closeOverlay();
  ...
  function getPersonaNote(name) {
    return extension_settings[extensionName].personaNotes?.[name] || "";
  }
  function setPersonaNote(name, note) {
    if (!extension_settings[extensionName].personaNotes)
      extension_settings[extensionName].personaNotes = {};
    if (note) {
      extension_settings[extensionName].personaNotes[name] = note;
    } else {
      delete extension_settings[extensionName].personaNotes[name];
    }
    getContext().saveSettingsDebounced();
  }
  ...
  function getCurrentPresetName() {
    try {
      const pm = getContext().getPresetManager();
      if (!pm || !pm.select) return null;
      const selected = pm.select.find("option:selected");
  ...
  function showMainPopup() {
    ...
    popup.find("#cfm-persona-left-sort-btn").on("click touchend", (e) => {
      e.preventDefault();
      e.stopPropagation();
      const wrapper = $("#cfm-persona-left-sort-wrapper");
      const topFolders = getResTopLevelFolders("personas");
      $(".cfm-sort-dropdown").remove();
      const dropdown = createResSortDropdown(
        "personas",
        personaLeftSortMode,
        ...
        (mode) => {
          if (mode === "revert") {
            ...
            personaLeftSortMode = null;
          } else {
            applyResSortToFolders("personas", topFolders, mode);
            personaLeftSortMode = mode;
          }
          renderPersonasView();
          dropdown.remove();
        },
    ...
    popup.find("#cfm-persona-right-sort-btn").on("click touchend", (e) => {
      e.preventDefault();
      e.stopPropagation();
      const wrapper = $("#cfm-persona-right-sort-wrapper");
      const currentFolder = selectedPersonaFolder;
      const childFolders =
        currentFolder &&
        currentFolder !== "__ungrouped__" &&
        currentFolder !== "__favorites__"
          ? getResChildFolders("personas", currentFolder)
          : [];
      $(".cfm-sort-dropdown").remove();
      const dropdown = $(`<div class="cfm-sort-dropdown cfm-sort-open">
      ...
      dropdown.find('[data-sort="revert"]').on("click touchend", (ev) => {
        ev.preventDefault();
        ev.stopPropagation();
        if (personaRightSortMode === null && !personaSortSnapshot) return;
        personaRightSortMode = null;
  ...
  function selectPersona(avatarId) {
    ...
    if (avatarBlock.length > 0) {
      ...
      // 尝试在酒馆原生 persona 面板中找到对应的 avatar 并触发点击
      const block = $("#user_avatar_block").find(
  ...
  async function renderPersonasView() {
    ...
    const personaFavNode = $(`
      <div class="cfm-tnode cfm-tnode-favorites ${selectedPersonaFolder === "__favorites__" ? "cfm-tnode-selected" : ""}" data-id="__favorites__" style="padding-left:10px;">
        <span class="cfm-tnode-arrow cfm-arrow-hidden"><i class="fa-solid fa-caret-right"></i></span>
        <span class="cfm-tnode-icon"><i class="fa-solid fa-star" style="color:#f9e2af;"></i></span>
        <span class="cfm-tnode-label">收藏</span>
    ...
    function renderResTreeNode(container, folderId, depth) {
      ...
      const hasChildren = children.length > 0;
      const isExpanded = personaExpandedNodes.has(folderId);
      const isSelected = selectedPersonaFolder === folderId;
      ...
      node.find(".cfm-tnode-arrow").on("click", (e) => {
        e.stopPropagation();
        if (!hasChildren) return;
        if (personaExpandedNodes.has(folderId))
          personaExpandedNodes.delete(folderId);
        else personaExpandedNodes.add(folderId);
        renderPersonasView();
      });
    ...
    if (selectedPersonaFolder === "__favorites__") {
      ...
      childFolders = sortResFolders(
        "personas",
        getResChildFolders("personas", selectedPersonaFolder),
      );
      const path = getResFolderPath("personas", selectedPersonaFolder)
    ...
    // 应用右栏排序
    if (personaRightSortMode && displayItems.length > 0) {
      displayItems = sortResItems(displayItems, personaRightSortMode, (p) =>
        typeof p === "string" ? p : p.name,
      );
    }
    ...
    if (
      ...
      rightList.on("dragleave", (e) => {
        if (
          $(e.relatedTarget).closest("#cfm-persona-right-list").length === 0
        ) {
          rightList.removeClass("cfm-right-list-drop-target");
        }
      });
  ...
  function executePersonaSearch() {
    ...
    getCurrentPersonas().then((personas) => {
      ...
      const tree = getResFolderTree("personas");
      let searchPool = [];
      if (scope === "current" && selectedPersonaFolder) {
        if (selectedPersonaFolder === "__favorites__") {
          const favs = getResFavorites("personas");
  ...
  async function renderRegexView() {
      ...
      function renderGlobalFolderNodes(parentId, level) {
        ...
        for (const fid of childIds) {
          ...
          if (hasChildren && isExpanded)
            renderGlobalFolderNodes(fid, level + 1);
        }
      }
    ...
    if (isScopedExp && charsWithRegex.size > 0) {
      ...
      function renderCharFolderNodes(parentId, level) {
        ...
        childFolderIds = childFolderIds.filter((id) =>
        ...
        for (const fid of childFolderIds) {
          ...
          const hasChildren =
            subChildren.length > 0 || charsInFolder.length > 0;
          regexAllNodeIds.push(nodeId);
          html += buildRegexTreeNodeHtml(
            nodeId,
            displayName,
    ...
    // --- 辅助：构建右栏子文件夹行HTML ---
    function buildRegexFolderRowHtml(nodeId, displayName, count) {
      return `
        <div class="cfm-row cfm-row-folder" data-target-node="${escapeHtml(nodeId)}">
          <div class="cfm-row-icon"><i class="fa-solid fa-folder"></i></div>
          <div class="cfm-row-name">${escapeHtml(displayName)}</div>
          <div class="cfm-row-meta">${count} 个脚本</div>
        </div>
      `;
    }

    // --- 辅助：构建右栏预设/角色子项行HTML ---
    function buildRegexItemRowHtml(nodeId, displayName, icon, count) {
      return `
        <div class="cfm-row cfm-row-folder" data-target-node="${escapeHtml(nodeId)}">
          <div class="cfm-row-icon"><i class="fa-solid ${icon}" style="font-size:20px;color:#8b9dfc;"></i></div>
          <div class="cfm-row-name">${escapeHtml(displayName)}</div>
          <div class="cfm-row-meta">${count} 个脚本</div>
        </div>
      `;
    ...
    // --- 绑定树节点点击事件 ---
    treeEl.find(".cfm-tnode").on("click", function (e) {
      const nodeId = $(this).data("node-id");

      // 箭头点击：展开/折叠
      if (
        $(e.target).closest(".cfm-tnode-arrow").length &&
        !$(e.target).closest(".cfm-arrow-hidden").length
      ) {
      ...
      // 选中节点
      selectedRegexNode = nodeId;
      treeEl.find(".cfm-tnode-selected").removeClass("cfm-tnode-selected");
      $(this).addClass("cfm-tnode-selected");
      ...
      if (nodeId === "rx-global") {
        ...
        let contentHtml = "";
        // 子文件夹
        const topGFolders = Object.keys(folderTree).filter(
          (id) => !folderTree[id].parentId,
        );
        for (const fid of topGFolders) {
          const count = (function countR(f) {
            let c = globalScripts.filter(
              (s) => globalGroups[s.id] === f,
            ).length;
            for (const ch of Object.keys(folderTree).filter(
              (i) => folderTree[i].parentId === f,
            ))
              c += countR(ch);
            return c;
          })(fid);
          contentHtml += buildRegexFolderRowHtml(
            "rx-gf-" + fid,
            folderTree[fid]?.displayName || fid,
            count,
          );
        }
        // 直属脚本（未分组的）
        const directScripts =
          topGFolders.length > 0
            ? globalScripts.filter(
                (s) => !globalGroups[s.id] || !folderTree[globalGroups[s.id]],
              )
            : globalScripts;
        directScripts.forEach((s) => {
          contentHtml += buildRegexScriptRowHtml(s, 0, "");
        });
        setRightPane("全局正则", globalScripts.length, contentHtml);
        ...
        setRightPane("全局正则 — 未归类", uncat.length, contentHtml);
      } else if (nodeId.startsWith("rx-gf-")) {
        const fid = nodeId.slice(6);
        let contentHtml = "";
        // 子文件夹行
        const childFids = Object.keys(folderTree).filter(
        ...
        for (const cfid of childFids) {
          ...
          contentHtml += buildRegexFolderRowHtml(
            "rx-gf-" + cfid,
            folderTree[cfid]?.displayName || cfid,
            count,
          );
        }
        // 直属脚本行
        const directScripts = globalScripts.filter(
          (s) => globalGroups[s.id] === fid,
        );
        directScripts.forEach((s) => {
          contentHtml += buildRegexScriptRowHtml(s, 0, "");
        });
        const totalItems = childFids.length + directScripts.length;
        setRightPane(
      ...
      // == 预设正则相关节点 ==
        ...
        const markedFolders2 = buildPrunedFolderSet(
          "presets",
          new Set(presetsWithRegex.keys()),
        );
        // 顶层文件夹
        const topPFolders = Object.keys(presetTree2).filter(
          (id) => !presetTree2[id].parentId && markedFolders2.has(id),
        );
        for (const fid of sortResFolders("presets", topPFolders)) {
          contentHtml += buildRegexFolderRowHtml(
            "rx-pf-" + fid,
            getResFolderDisplayName("presets", fid),
            0,
          );
        }
        // 未归类的预设项
        const uncatPresetNames = [];
        for (const [pName] of presetsWithRegex) {
          const g = presetGroups2[pName];
          if (!g || !presetTree2[g]) uncatPresetNames.push(pName);
        }
        for (const pName of uncatPresetNames) {
          const sc = presetsWithRegex.get(pName);
          contentHtml += buildRegexItemRowHtml(
            "rx-pi-" + pName,
            pName,
            "fa-file-code",
            sc.length,
        ...
        setRightPane("预设正则", presetsWithRegex.size, contentHtml);
      } else if (nodeId.startsWith("rx-pi-")) {
        const pName = nodeId.slice(6);
        const scripts = presetsWithRegex.get(pName) || [];
        let contentHtml = "";
        scripts.forEach((s) => {
          contentHtml += buildRegexScriptRowHtml(s, 2, pName);
        });
        setRightPane("预设正则 — " + pName, scripts.length, contentHtml);
        ...
        if (fid !== null) {
          ...
          const childFids = Object.keys(presetTree2).filter(
            (id) => presetTree2[id].parentId === fid && markedFolders2.has(id),
          );
          for (const cfid of sortResFolders("presets", childFids)) {
            contentHtml += buildRegexFolderRowHtml(
              "rx-pf-" + cfid,
              getResFolderDisplayName("presets", cfid),
              0,
            );
          }
        }
        ...
        for (const [pName] of presetsWithRegex) {
          if (fid === null) {
            const g = presetGroups2[pName];
            if (!g || !presetTree2[g]) directPresets.push(pName);
          } else {
            if (presetGroups2[pName] === fid) directPresets.push(pName);
          }
        }
        for (const pName of directPresets) {
          const sc = presetsWithRegex.get(pName);
          contentHtml += buildRegexItemRowHtml(
            "rx-pi-" + pName,
            pName,
        ...
        const totalItems = directPresets.length;
        setRightPane("预设正则 — " + title, totalItems, contentHtml);
      }
      ...
      // == 角色正则相关节点 ==
        ...
        // 顶层文件夹
        const topCFolders = sortFolders(
          getTopLevelFolders().filter((id) => markedFolders2.has(id)),
        );
        for (const fid of topCFolders) {
          contentHtml += buildRegexFolderRowHtml(
            "rx-sf-" + fid,
            getTagName(fid),
            0,
        ...
        setRightPane("角色正则", charsWithRegex.size, contentHtml);
        ...
        if (info) {
          ...
          info.scripts.forEach((s) => {
            contentHtml += buildRegexScriptRowHtml(s, 1, info.name);
          });
          setRightPane(
            "角色正则 — " + info.name,
            info.scripts.length,
        ...
        if (fid !== null) {
          ...
          const childFids = getChildFolders(fid).filter((id) =>
            markedFolders2.has(id),
          );
          for (const cfid of sortFolders(childFids)) {
            contentHtml += buildRegexFolderRowHtml(
              "rx-sf-" + cfid,
              getTagName(cfid),
              0,
        ...
        const directChars =
          fid === null
            ? getUncategorizedCharacters().filter((c) =>
                charsWithRegex.has(c.avatar),
              )
            : getCharactersInFolder(fid).filter((c) =>
                charsWithRegex.has(c.avatar),
              );
        ...
        const totalItems = directChars.length;
    ...
    rightList
      ...
      .on(
        "click.rxnav",
        ".cfm-row-folder[data-target-node], .cfm-row[data-target-node]",
        function (e) {
          e.preventDefault();
          const targetNodeId = $(this).data("target-node");
          if (!targetNodeId) return;
          // 展开路径上的所有父节点
          if (targetNodeId.startsWith("rx-gf-")) {
            regexExpandedNodes.add("rx-global");
            // 展开上级全局文件夹
            let cur = folderTree[targetNodeId.slice(6)]?.parentId;
            while (cur) {
              regexExpandedNodes.add("rx-gf-" + cur);
              cur = folderTree[cur]?.parentId;
            }
          } else if (targetNodeId.startsWith("rx-pf-")) {
            regexExpandedNodes.add("rx-preset");
            const presetTree2 = getResFolderTree("presets");
            let cur = presetTree2[targetNodeId.slice(6)]?.parentId;
            while (cur) {
              regexExpandedNodes.add("rx-pf-" + cur);
              cur = presetTree2[cur]?.parentId;
            }
          } else if (targetNodeId.startsWith("rx-pi-")) {
            regexExpandedNodes.add("rx-preset");
            // 展开所在文件夹
            const presetGroups2 = getResourceGroups("presets");
            const presetTree2 = getResFolderTree("presets");
            let cur = presetGroups2[targetNodeId.slice(6)];
            while (cur) {
              regexExpandedNodes.add("rx-pf-" + cur);
              cur = presetTree2[cur]?.parentId;
            }
            if (!cur) regexExpandedNodes.add("rx-preset-uncat");
          } else if (targetNodeId.startsWith("rx-sf-")) {
            regexExpandedNodes.add("rx-scoped");
            let cur = config.folders[targetNodeId.slice(6)]?.parentId;
            while (cur) {
              regexExpandedNodes.add("rx-sf-" + cur);
              cur = config.folders[cur]?.parentId;
            }
          } else if (targetNodeId.startsWith("rx-si-")) {
            regexExpandedNodes.add("rx-scoped");
            // 展开所在角色文件夹
            const tagMap = getTagMap();
            const folderTagIdSet = new Set(getFolderTagIds());
            const charTags = tagMap[targetNodeId.slice(6)] || [];
            for (const tagId of charTags) {
              if (!folderTagIdSet.has(tagId)) continue;
              let cur = tagId;
              while (cur) {
            ...
            regexExpandedNodes.add("rx-scoped-uncat");
          }
          selectedRegexNode = targetNodeId;
          renderRegexView();
        },
      );
    ...
    if (targetNode.length) {
      ...
      // 回退到第一个节点
      selectedRegexNode = "rx-global";
      treeEl.find(".cfm-tnode").first().trigger("click");
    }
  }
