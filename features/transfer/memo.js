...
export function createEntryTransferMemoApiCore(deps) {
  ...
  function setEntryTransferMemoGroupFavorite(id, favorite) {
    return updateEntryTransferMemoGroup(id, { favorite: !!favorite });
  }
  ...
  /**
   * 将收藏组转为临时组（全部缝合前调用，避免误操作收藏组）
   */
  function convertFavoriteGroupToPending(id) {
    return setEntryTransferMemoGroupFavorite(id, false);
  }

  return {
    addEntryTransferMemoGroup,
    convertFavoriteGroupToPending,
    ...
    setEntryTransferMemoGroupFavorite,
